...
namespace Unity.LiveCapture.ARKitFaceCapture
{
    ...
    [ExcludeFromPreset]
    [AddComponentMenu("Live Capture/ARKit Face Capture/Custom ARKit Face Actor")]
    [ExecuteAlways]
    [RequireComponent(typeof(Animator))]
    public class CustomARKitFaceActor : MonoBehaviour
    {
        enum FaceChannels
        {
            /// <summary>
        ...
        CustomARKitFaceMapper m_Mapper = null;
        ...
        FaceChannels m_EnabledChannels = FaceChannels.All;
        ...
        /// </summary>
        public CustomARKitFaceMapper Mapper => m_Mapper;
        ...
        public void SetMapper(CustomARKitFaceMapper mapper)
        {
            m_Mapper = mapper;
            ClearCache();
        }
        ...
        public void UpdateRig(bool continuous)
        {
            if (m_Mapper != null)
            {
                ...
                {
                    channels &= ~FaceChannels.BlendShapes;
                }
                if (!m_HeadPositionEnabled)
                {
                    channels &= ~FaceChannels.HeadPosition;
                }
                if (!m_HeadOrientationEnabled)
                {
                    channels &= ~FaceChannels.HeadRotation;
                }
                if (!m_EyeOrientationEnabled)
                {
                    channels &= ~FaceChannels.Eyes;
                }

                // apply the active face channels
                if (channels.HasFlag(FaceChannels.BlendShapes))
                {
                    m_Mapper.ApplyBlendShapesToRig(
                        this,
                        m_Cache,
                        ref m_BlendShapes,
                        continuous && m_LastChannels.HasFlag(FaceChannels.BlendShapes)
                    );
                }
                if (channels.HasFlag(FaceChannels.HeadPosition))
                {
                    m_Mapper.ApplyHeadPositionToRig(
                        this,
                        m_Cache,
                        ref m_HeadPosition,
                        continuous && m_LastChannels.HasFlag(FaceChannels.HeadPosition)
                    );
                }
                if (channels.HasFlag(FaceChannels.HeadRotation))
                {
                    ...
                    m_Mapper.ApplyHeadRotationToRig(
                        ...
                        m_Cache,
                        ref headRotation,
                        continuous && m_LastChannels.HasFlag(FaceChannels.HeadRotation)
                    );
                }
                if (channels.HasFlag(FaceChannels.Eyes))
                {
                    ...
                    m_Mapper.ApplyEyeRotationToRig(
                        ...
                        continuous && m_LastChannels.HasFlag(FaceChannels.Eyes)
                    );
                }
