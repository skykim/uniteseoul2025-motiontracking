...
public class CameraControl : MonoBehaviour
{
    ...
    void HandleInput()
    {
        ...
        {
            m_DownFromGUI = CheckGuiRaycastObjects();
            if (!m_DownFromGUI)
            {
                // Check if
        ...
        if (Input.GetMouseButtonUp(orientationControlButton) || Input.GetMouseButtonUp(pedestalControlButton))
        {
            m_DownFromGUI = false;
        }

        if (!m_DownFromGUI && Input.GetMouseButton(orientationControlButton))
        {
        ...
        if (!m_DownFromGUI && Input.GetMouseButton(pedestalControlButton))
        {
    ...
    void FollowTarget()
    {
        ...
        orientationTarget = Quaternion.LookRotation(m_target.forward).eulerAngles;

        targetOrientationOffset = Quaternion.Inverse(Quaternion.Euler(orientationTarget)) * m_target.rotation;

        targetDistanceFromAnchor = (Vector3.Cross(anchor.position - m_target.position, Vector3.up)).magnitude;
        targetDistanceFromAnchorLastFollow = targetDistanceFromAnchor;

        pedestalTarget = (Quaternion.Euler(orientationTarget) * Vector3.forward * targetDistanceFromAnchor).y + m_target.position.y;
